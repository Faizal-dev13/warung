...
@section('page_description','Ringkasan performa toko hari ini. Admin bisa langsung cek produk, promo aktif, dan pesanan terbaru dari halaman ini.')
@section('content')
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse($latestOrders as $order)
                        <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-800/40">
                            <td class="px-5 py-4 font-extrabold"><a class="text-slate-950 hover:text-blue-600 dark:text-white" href="{{ route('admin.orders.show',$order) }}">{{ $order->invoice_number }}</a></td>
                            <td><b>{{ $order->customer_name }}</b></td>
                            <td class="font-bold">Rp {{ number_format($order->total,0,',','.') }}</td>
                            <td><span class="rounded-full bg-amber-50 px-3 py-1 text-xs font-extrabold capitalize text-amber-700 dark:bg-amber-400/10 dark:text-amber-300">{{ str_replace('_',' ',$order->status) }}</span></td>
                            <td class="text-slate-500 dark:text-slate-400">{{ $order->created_at->format('d M Y H:i') }}</td>
                            <td class="pr-5 text-right"><a href="{{ route('admin.orders.show',$order) }}" class="inline-flex items-center gap-1 rounded-xl bg-slate-950 px-3 py-2 text-xs font-extrabold text-white dark:bg-white dark:text-slate-950">Detail <i class="ph ph-arrow-right"></i></a></td>
                        </tr>
                    @empty
