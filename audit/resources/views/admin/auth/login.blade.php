    ...
    <main class="grid min-h-screen place-items-center overflow-hidden p-4">
        ...
        <form method="POST" action="{{ route('admin.login.submit') }}" class="relative w-full max-w-md rounded-[2rem] border border-white/10 bg-white/10 p-6 shadow-2xl backdrop-blur">@csrf
            ...
            <div class="grid gap-4">
                <label><span class="mb-2 block text-sm font-bold">Email Admin</span><input name="email" type="email" value="{{ old('email') }}" placeholder="admin@email.com" class="w-full rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm font-semibold outline-none transition placeholder:text-white/35 focus:border-white/40 focus:bg-white/15" required autofocus></label>
