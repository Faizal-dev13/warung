...
@section('page_description','Upload foto produk, isi harga, deskripsi, dan status. Admin tidak perlu mengatur warna, gradient, atau kode icon.')
@section('content')
@php
...
<form method="POST" enctype="multipart/form-data" action="{{ $product->exists ? route('admin.products.update',$product) : route('admin.products.store') }}" class="space-y-5">@csrf @if($product->exists)@method('PUT')@endif
    <section class="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="grid gap-4 lg:grid-cols-2">
            ...
            <label class="block"><span class="mb-2 block text-sm font-extrabold">Kategori</span><select name="category_id" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none transition focus:border-slate-950 focus:bg-white dark:border-slate-700 dark:bg-slate-800 dark:focus:border-white/40" required><option value="">Pilih kategori</option>@foreach($categories as $category)<option value="{{ $category->id }}" @selected(old('category_id',$product->category_id)==$category->id)>{{ $category->name }}</option>@endforeach</select></label>
        ...
        <div class="grid gap-5 lg:grid-cols-[.9fr_1.1fr] lg:items-stretch">
            <div class="overflow-hidden rounded-[1.35rem] border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-800/60">
                <div class="relative aspect-[4/3] w-full overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900">
                    <img data-image-preview src="{{ $productImageUrl ?: '' }}" alt="Preview foto produk" class="{{ $productImageUrl ? '' : 'hidden' }} h-full w-full object-cover">
    ...
    <section class="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-3">
            <label class="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 p-4 dark:border-slate-800"><input type="checkbox" name="is_latest" value="1" class="mt-1 h-4 w-4" @checked(old('is_latest',$product->is_latest))><span><b class="block text-sm">Tampilkan di Terbaru</b><small class="text-slate-500 dark:text-slate-400">Muncul di section produk terbaru.</small></span></label>
