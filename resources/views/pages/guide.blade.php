...
<section class="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
    <div class="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-soft dark:border-white/10 dark:bg-white/5">
        <p class="font-bold text-teal-600 dark:text-teal-400">Panduan Pengguna</p>
        ...
        <div class="mt-8 grid gap-4">
            @foreach([
                ...
                ['Masukkan keranjang', 'Klik tombol tambah pada produk atau halaman detail.'],
                ['Isi data checkout', 'Buka keranjang, isi nama, nomor HP opsional, voucher, dan catatan.'],
                ['Kirim ke WhatsApp', 'Klik checkout dan pesanan akan diteruskan ke WhatsApp untuk dikonfirmasi.'],
            ] as $index => $step)
                <div class="flex gap-4 rounded-3xl border border-slate-200 p-5 dark:border-white/10"><span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-teal-700 font-bold text-white dark:bg-teal-500 dark:text-white">{{ $index + 1 }}</span><div><h3 class="font-bold">{{ $step[0] }}</h3><p class="mt-1 text-sm leading-7 text-slate-500 dark:text-slate-400">{{ $step[1] }}</p></div></div>
            @endforeach
        </div>
    </div>
</section>
