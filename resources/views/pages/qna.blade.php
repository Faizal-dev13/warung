@extends('layouts.app')
@section('title', ($settings['qna_title'] ?? 'QnA').' - '.($settings['store_name'] ?? config('store.name')))
@section('content')
@php
    ...
    <div class="overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-soft sm:p-8 lg:p-10">
        <div class="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
                ...
                <h1 class="mt-5 text-3xl font-extrabold tracking-tight sm:text-5xl">{{ $title }}</h1>
                <p class="mt-4 max-w-2xl text-sm leading-7 text-white/70 sm:text-base">{{ $subtitle }}</p>
            </div>
            @if($whatsappNumber)
                <a href="https://wa.me/{{ $whatsappNumber }}" class="inline-flex items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-extrabold text-slate-950 transition hover:-translate-y-0.5 hover:shadow-lg"><i class="ph-fill ph-whatsapp-logo"></i> Tanya Admin</a>
            @endif
        </div>
    </div>
    ...
    <div class="mt-6 grid gap-3 sm:mt-8">
        @forelse($qnas as $qna)
            <details class="group rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition open:border-teal-200 open:bg-teal-50/40 dark:border-white/10 dark:bg-white/5 dark:open:border-teal-400/30 dark:open:bg-teal-400/10 sm:p-6">
                ...
                <p class="mt-4 text-sm leading-7 text-slate-600 dark:text-slate-300 sm:text-base">{{ $qna->answer }}</p>
            </details>
        @empty
