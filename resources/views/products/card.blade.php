...
<article class="store-product-card group flex h-full min-w-0 flex-col overflow-hidden rounded-[1.35rem] border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-teal-200 hover:shadow-soft dark:border-white/10 dark:bg-white/5 dark:hover:border-teal-400/30">
    <a href="{{ route('products.show', $product->slug) }}" class="block p-3 pb-0 sm:p-4 sm:pb-0" aria-label="Lihat detail {{ $product->name }}">
        <div class="store-product-media aspect-[4/3] relative overflow-hidden rounded-[1.05rem] bg-slate-100 ring-1 ring-slate-100 dark:bg-white/10 dark:ring-white/10 sm:rounded-[1.25rem]">
            @if($productImageUrl)
                <img src="{{ $productImageUrl }}" alt="{{ $product->name }}" class="store-product-photo absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy">
            @else
    ...
    <div class="store-product-body flex min-w-0 flex-1 flex-col p-3 sm:p-4">
        <p class="store-product-category truncate text-[11px] font-extrabold uppercase tracking-wide text-teal-600 dark:text-teal-400 sm:text-xs">{{ $product->category?->name ?? 'Produk' }}</p>

        <a href="{{ route('products.show', $product->slug) }}" class="store-product-title mt-1.5 line-clamp-2 min-h-[2.25rem] text-sm font-extrabold leading-tight text-slate-950 transition group-hover:text-teal-600 dark:text-white dark:group-hover:text-teal-400 sm:mt-2 sm:min-h-[3rem] sm:text-lg sm:leading-snug">
            {{ $product->name }}
        </a>
        ...
        <div class="store-product-footer mt-auto pt-3">
            <div class="min-w-0">
                @if($product->old_price > $product->price)
                    <p class="store-product-old-price truncate text-[11px] font-semibold text-slate-400 line-through sm:text-sm">{{ Money::rupiah($product->old_price) }}</p>
                @endif
                <p class="store-product-price truncate text-sm font-extrabold text-slate-950 dark:text-white sm:text-xl">{{ Money::rupiah($product->price) }}</p>
            </div>
