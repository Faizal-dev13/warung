...
<article class="store-product-card group flex h-full min-w-0 flex-col overflow-hidden rounded-[1.35rem] border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-soft dark:border-white/10 dark:bg-white/5 dark:hover:border-blue-400/30">
    ...
    <div class="store-product-body flex min-w-0 flex-1 flex-col p-3 sm:p-4">
        <p class="store-product-category truncate text-[11px] font-extrabold uppercase tracking-wide text-blue-600 dark:text-blue-400 sm:text-xs">{{ $product->category?->name ?? 'Produk' }}</p>

        <a href="{{ route('products.show', $product->slug) }}" class="store-product-title mt-1.5 line-clamp-2 min-h-[2.25rem] text-sm font-extrabold leading-tight text-slate-950 transition group-hover:text-blue-600 dark:text-white dark:group-hover:text-blue-400 sm:mt-2 sm:min-h-[3rem] sm:text-lg sm:leading-snug">
            {{ $product->name }}
        </a>
