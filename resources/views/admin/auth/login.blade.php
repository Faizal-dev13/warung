...
<body class="min-h-screen bg-slate-950 font-sans text-white">
    <main class="grid min-h-screen overflow-hidden lg:grid-cols-[1fr_480px]">
        ...
        <section class="relative grid min-h-screen place-items-center p-4 sm:p-6">
            ...
            <form method="POST" action="{{ route('admin.login.submit') }}" class="relative w-full max-w-md rounded-[2rem] border border-white/10 bg-white/[.08] p-5 shadow-soft backdrop-blur sm:p-7">
                @csrf
                ...
                @if(session('error'))
                    <div class="mb-4 rounded-2xl border border-red-300/20 bg-red-500/20 px-4 py-3 text-sm font-bold text-red-100">{{ session('error') }}</div>
                @endif
                @if(session('success'))
                    <div class="mb-4 rounded-2xl border border-emerald-300/20 bg-emerald-500/20 px-4 py-3 text-sm font-bold text-emerald-100">{{ session('success') }}</div>
                @endif

                <div class="grid gap-4">
                    <label><span class="mb-2 block text-sm font-bold">Email Admin</span><input name="email" type="email" value="{{ old('email') }}" placeholder="admin@email.com" class="w-full rounded-2xl border border-white/10 bg-white/10 px-4 py-3.5 text-sm font-semibold outline-none transition placeholder:text-white/35 focus:border-teal-300/70 focus:bg-white/15" required autofocus></label>
