...
<form method="POST" enctype="multipart/form-data" action="{{ $banner->exists ? route('admin.banners.update',$banner) : route('admin.banners.store') }}" class="space-y-5">@csrf @if($banner->exists)@method('PUT')@endif
    ...
    <section class="rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900">
        ...
        <div class="grid gap-4 lg:grid-cols-2">
            <label class="block"><span class="mb-2 block text-sm font-extrabold">Urutan Slide</span><input name="sort_order" type="number" min="1" value="{{ old('sort_order', $banner->exists ? max(1, (int) $banner->sort_order) : ($nextSortOrder ?? 1)) }}" class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold outline-none transition focus:border-teal-600 focus:bg-white dark:border-slate-700 dark:bg-slate-800"><small class="mt-1 block text-xs leading-5 text-slate-400">Jika diisi 2, slide yang sebelumnya posisi 2 dan setelahnya otomatis bergeser.</small></label>
