...
@php
    $menus = [
        ...
        ['Testimoni','admin.testimonials.index','ph-chat-teardrop-text'],
    ...
    <div class="admin-scrollbar flex-1 overflow-y-auto px-4 py-5">
        ...
        <nav class="mt-6 grid gap-1.5">
            @foreach($menus as [$label,$route,$icon])
                ...
                @endphp
                <a href="{{ route($route) }}" class="group flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold transition {{ $isActive ? 'bg-teal-600 text-white shadow-lg shadow-teal-950/30' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">
                    <span class="grid h-9 w-9 place-items-center rounded-xl {{ $isActive ? 'bg-white/[.16] text-white' : 'bg-white/[.06] text-slate-300 group-hover:bg-white/10 group-hover:text-white' }}">
        ...
        <div class="mb-6 overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white/92 p-5 shadow-soft backdrop-blur dark:border-slate-800 dark:bg-slate-900/85 sm:p-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div class="min-w-0">
                    ...
                    <h1 class="text-2xl font-extrabold tracking-tight text-slate-950 dark:text-white sm:text-3xl lg:text-4xl">@yield('page_title', 'Dashboard')</h1>
                    @hasSection('page_description')
                        <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500 dark:text-slate-400">@yield('page_description')</p>
                    @else
