...
<article class="store-product-card group flex h-full min-w-0 flex-col overflow-hidden rounded-[1.35rem] border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-teal-200 hover:shadow-soft dark:border-white/10 dark:bg-white/5 dark:hover:border-teal-400/30">
    <a href="<?php echo e(route('products.show', $product->slug)); ?>" class="block p-3 pb-0 sm:p-4 sm:pb-0" aria-label="Lihat detail <?php echo e($product->name); ?>">
        <div class="store-product-media aspect-[4/3] relative overflow-hidden rounded-[1.05rem] bg-slate-100 ring-1 ring-slate-100 dark:bg-white/10 dark:ring-white/10 sm:rounded-[1.25rem]">
            <?php if($productImageUrl): ?>
                <img src="<?php echo e($productImageUrl); ?>" alt="<?php echo e($product->name); ?>" class="store-product-photo absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105" loading="lazy" decoding="async">
    ...
    <div class="store-product-body flex min-w-0 flex-1 flex-col p-3 sm:p-4">
        <p class="store-product-category truncate text-[11px] font-extrabold uppercase tracking-wide text-teal-700 dark:text-teal-300 sm:text-xs"><?php echo e($product->category?->name ?? 'Produk'); ?></p>

        <a href="<?php echo e(route('products.show', $product->slug)); ?>" class="store-product-title mt-1.5 line-clamp-2 min-h-[2.25rem] text-sm font-extrabold leading-tight text-slate-950 transition group-hover:text-teal-700 dark:text-white dark:group-hover:text-teal-300 sm:mt-2 sm:min-h-[3rem] sm:text-lg sm:leading-snug">
        ...
        <div class="store-product-footer mt-auto pt-3">
            <div class="min-w-0 rounded-2xl bg-slate-50 p-3 dark:bg-white/5">
                <?php if($displayOldPrice > $displayPrice): ?>
                    <p class="store-product-old-price truncate text-[11px] font-semibold text-slate-400 line-through sm:text-sm"><?php echo e(Money::rupiah($displayOldPrice)); ?></p>
                <?php endif; ?>
                <?php if($hasVariants): ?>
                    <p class="text-[10px] font-extrabold uppercase tracking-wide text-emerald-600 dark:text-emerald-300">Mulai dari</p>
                <?php endif; ?>
                <p class="store-product-price truncate text-sm font-extrabold text-slate-950 dark:text-white sm:text-xl"><?php echo e(Money::rupiah($displayPrice)); ?></p>
            </div>

            <div class="store-product-actions mt-3 grid grid-cols-[1fr_2.5rem] gap-2 sm:grid-cols-[1fr_3rem]">
                <?php if($hasVariants): ?>
                    <a href="<?php echo e(route('products.show', $product->slug)); ?>" class="store-product-add-button inline-flex h-10 w-full items-center justify-center gap-1.5 rounded-2xl bg-slate-950 px-3 text-xs font-extrabold text-white transition hover:scale-[1.02] dark:bg-white dark:text-slate-950 sm:h-12 sm:text-sm">
