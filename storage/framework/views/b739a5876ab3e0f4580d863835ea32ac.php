...
<?php $__env->startSection('page_description','Ringkasan performa toko, pesanan terbaru, dan aksi penting yang perlu ditindaklanjuti.'); ?>
...
<section class="relative overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-950 p-5 text-white shadow-soft dark:border-slate-800 sm:p-6 lg:p-7">
    ...
    <div class="relative grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center">
        ...
        <div class="grid gap-3 sm:grid-cols-2 lg:min-w-[360px]">
            <a href="<?php echo e(route('admin.orders.index')); ?>" class="group rounded-3xl border border-white/10 bg-white/10 p-4 transition hover:-translate-y-0.5 hover:bg-white/15">
                ...
                <p class="mt-3 text-3xl font-extrabold"><?php echo e($totalOrders); ?></p>
                <p class="mt-1 text-xs font-semibold text-slate-300">Total pesanan masuk</p>
            </a>
            <a href="<?php echo e(route('admin.orders.index')); ?>" class="group rounded-3xl border border-emerald-300/20 bg-emerald-400/10 p-4 transition hover:-translate-y-0.5 hover:bg-emerald-400/15">
                <div class="flex items-center justify-between gap-3">
                ...
                <p class="mt-3 text-3xl font-extrabold"><?php echo e($pendingOrders); ?></p>
...
<section class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <?php $__currentLoopData = $summaryCards; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $card): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <article class="group relative overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-soft transition duration-300 hover:-translate-y-1 hover:shadow-card dark:border-slate-800 dark:bg-slate-900">
            <div class="absolute inset-x-0 top-0 h-1 bg-gradient-to-r <?php echo e($card['accent']); ?>"></div>
            <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                    <p class="text-sm font-bold text-slate-500 dark:text-slate-400"><?php echo e($card['label']); ?></p>
                    <p class="mt-2 text-3xl font-extrabold tracking-tight text-slate-950 dark:text-white"><?php echo e(number_format($card['value'], 0, ',', '.')); ?></p>
                    <p class="mt-1 text-xs font-semibold leading-5 text-slate-400 dark:text-slate-500"><?php echo e($card['description']); ?></p>
                </div>
                <span class="grid h-12 w-12 shrink-0 place-items-center rounded-2xl ring-1 <?php echo e($card['tone']); ?>">
                    <i class="ph <?php echo e($card['icon']); ?> text-2xl"></i>
                </span>
            </div>
        </article>
    ...
    <div class="rounded-[1.7rem] border border-slate-200 bg-white shadow-soft dark:border-slate-800 dark:bg-slate-900">
        <div class="border-b border-slate-100 p-5 dark:border-slate-800 sm:p-6">
            <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <a href="<?php echo e(route('admin.orders.index')); ?>" class="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-extrabold text-slate-700 transition hover:-translate-y-0.5 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
                    Lihat Semua
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                        <?php $__empty_1 = true; $__currentLoopData = $latestOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr class="transition hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                                <td class="px-5 py-4">
                                    <a class="font-extrabold text-slate-950 transition hover:text-teal-700 dark:text-white dark:hover:text-teal-300" href="<?php echo e(route('admin.orders.show',$order)); ?>">
                                        <?php echo e($order->invoice_number); ?>

                                    </a>
                                </td>
                                <td class="px-3 py-4">
                                    <div class="font-bold text-slate-800 dark:text-slate-100"><?php echo e($order->customer_name); ?></div>
                                </td>
                                ...
                                <td class="px-3 py-4">
                                    <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-extrabold ring-1 <?php echo e($statusClasses[$order->status] ?? 'bg-slate-100 text-slate-600 ring-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:ring-slate-700'); ?>">
                                        <?php echo e($statusLabels[$order->status] ?? str_replace('_',' ',$order->status)); ?>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <a href="<?php echo e(route('admin.orders.show',$order)); ?>" class="inline-flex items-center gap-1 rounded-xl bg-teal-700 px-3.5 py-2 text-xs font-extrabold text-white shadow-lg shadow-teal-700/15 transition hover:-translate-y-0.5 hover:bg-teal-800 dark:bg-teal-500 dark:hover:bg-teal-400">
                                        Detail
                                        <i class="ph ph-arrow-right"></i>
                                    </a>
                                </td>
        ...
        <div class="grid gap-3 p-4 md:hidden">
            <?php $__empty_1 = true; $__currentLoopData = $latestOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <a href="<?php echo e(route('admin.orders.show',$order)); ?>" class="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm transition active:scale-[0.99] dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <p class="truncate text-sm font-extrabold text-slate-950 dark:text-white"><?php echo e($order->invoice_number); ?></p>
                            <p class="mt-1 truncate text-sm font-semibold text-slate-500 dark:text-slate-400"><?php echo e($order->customer_name); ?></p>
                        </div>
                        <span class="inline-flex shrink-0 items-center rounded-full px-3 py-1 text-[11px] font-extrabold ring-1 <?php echo e($statusClasses[$order->status] ?? 'bg-slate-100 text-slate-600 ring-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:ring-slate-700'); ?>">
                            <?php echo e($statusLabels[$order->status] ?? str_replace('_',' ',$order->status)); ?>
                    ...
                    <div class="mt-4 flex items-end justify-between gap-3">
                        <div>
                            ...
                            <p class="text-base font-extrabold text-slate-950 dark:text-white">Rp <?php echo e(number_format($order->total,0,',','.')); ?></p>
                        </div>
                        <div class="text-right">
                            <p class="text-xs font-bold text-slate-400">Tanggal</p>
                            <p class="text-xs font-bold text-slate-500 dark:text-slate-400"><?php echo e($order->created_at->format('d M Y H:i')); ?></p>
                        </div>
                    </div>
                </a>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
        ...
        <section class="rounded-[1.7rem] border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900 sm:p-6">
            ...
            <div class="mt-5 grid gap-3">
                <?php $__currentLoopData = $quickActions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $action): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <a href="<?php echo e($action['route']); ?>" class="group flex items-center gap-3 rounded-3xl border border-slate-200 p-4 transition hover:-translate-y-0.5 hover:bg-slate-50 hover:shadow-sm dark:border-slate-800 dark:hover:bg-slate-800/50">
                        <span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl <?php echo e($action['tone']); ?>">
                            <i class="ph <?php echo e($action['icon']); ?> text-xl"></i>
                        </span>
                        <span class="min-w-0 flex-1">
                            <span class="block text-sm font-extrabold text-slate-900 dark:text-white"><?php echo e($action['label']); ?></span>
                            <span class="mt-0.5 block text-xs font-semibold leading-5 text-slate-500 dark:text-slate-400"><?php echo e($action['description']); ?></span>
                        </span>
