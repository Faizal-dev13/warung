...
<?php $__env->startSection('page_description','Atur identitas toko, logo, kontak, WhatsApp, panduan, dan halaman QnA agar tampilan depan selalu mengikuti data terbaru.'); ?>
<?php $__env->startSection('content'); ?>
<form method="POST" action="<?php echo e(route('admin.settings.update')); ?>" enctype="multipart/form-data" class="space-y-5">
    <?php echo csrf_field(); ?>
    ...
    <section class="overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-soft dark:border-slate-800 dark:bg-slate-900">
        <div class="bg-slate-950 p-5 text-white sm:p-6">
            <div class="flex items-start gap-3">
                <span class="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-2xl bg-teal-600 text-white shadow-lg shadow-teal-950/30">
                    <?php if($logoUrl): ?>
                        <img src="<?php echo e($logoUrl); ?>" alt="Logo" class="h-full w-full object-cover">
                    <?php else: ?>
        ...
        <div class="grid gap-5 p-5 sm:p-6 lg:grid-cols-2">
            <label class="block">
                ...
                <input name="store_name" value="<?php echo e(old('store_name', $settings['store_name'] ?? '')); ?>" class="admin-input" required>
            </label>
            <label class="block">
                <span class="mb-2 block text-sm font-extrabold">Tagline</span>
                <input name="store_tagline" value="<?php echo e(old('store_tagline', $settings['store_tagline'] ?? '')); ?>" class="admin-input" required>
            </label>
            <label class="block">
                <span class="mb-2 block text-sm font-extrabold">Teks Kecil Header</span>
                <input name="header_subtitle" value="<?php echo e(old('header_subtitle', $settings['header_subtitle'] ?? '')); ?>" class="admin-input" placeholder="Produk digital siap checkout WA">
            </label>
            <label class="block">
                <span class="mb-2 block text-sm font-extrabold">Nomor WhatsApp Admin</span>
                <input name="whatsapp_number" value="<?php echo e(old('whatsapp_number', $settings['whatsapp_number'] ?? '')); ?>" class="admin-input" placeholder="6281234567890" required>
            ...
            <label class="block">
                ...
                <input name="support_email" type="email" value="<?php echo e(old('support_email', $settings['support_email'] ?? '')); ?>" class="admin-input" placeholder="support@domain.com">
            </label>
            <label class="block">
                ...
                <input name="business_address" value="<?php echo e(old('business_address', $settings['business_address'] ?? '')); ?>" class="admin-input" placeholder="Kota / alamat bisnis">
            </label>
        </div>
    </section>
    ...
    <section class="rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-soft dark:border-slate-800 dark:bg-slate-900 sm:p-6">
        ...
        <div class="grid gap-5 lg:grid-cols-2">
            <label class="block">
                ...
                <input name="guide_title" value="<?php echo e(old('guide_title', $settings['guide_title'] ?? '')); ?>" class="admin-input" required>
            </label>
            <label class="block">
                <span class="mb-2 block text-sm font-extrabold">Judul QnA</span>
                <input name="qna_title" value="<?php echo e(old('qna_title', $settings['qna_title'] ?? '')); ?>" class="admin-input" required>
            </label>
            <label class="block lg:col-span-2">
                <span class="mb-2 block text-sm font-extrabold">Deskripsi Panduan</span>
                <textarea name="guide_subtitle" rows="3" class="admin-textarea"><?php echo e(old('guide_subtitle', $settings['guide_subtitle'] ?? '')); ?></textarea>
            </label>
            <label class="block lg:col-span-2">
                <span class="mb-2 block text-sm font-extrabold">Deskripsi QnA</span>
                <textarea name="qna_subtitle" rows="3" class="admin-textarea"><?php echo e(old('qna_subtitle', $settings['qna_subtitle'] ?? '')); ?></textarea>
            </label>
        </div>
    </section>
    ...
    <div class="sticky bottom-4 z-10 flex flex-col-reverse gap-3 rounded-3xl border border-slate-200 bg-white/90 p-3 shadow-soft backdrop-blur dark:border-slate-800 dark:bg-slate-900/90 sm:flex-row sm:items-center sm:justify-end">
        <a href="<?php echo e(route('admin.dashboard')); ?>" class="admin-btn-secondary">Batal</a>
        <button class="admin-btn-primary"><i class="ph ph-check-circle"></i> Simpan Settings</button>
    </div>
</form>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/faizal/Projects/laravel/digitalkit/resources/views/admin/settings/edit.blade.php ENDPATH**/ ?>